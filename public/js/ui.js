...
const ui = {
  ...
  comicCard(comic) {
    ...
    // BacaKomik pakai 'cover', fallback ke image/thumbnail
    const thumb = comic.cover || comic.image || comic.thumbnail || '';
    const type  = comic.type  || '';
    // BacaKomik latest: field 'chapter' (string e.g. "Ch.90")
  ...
  renderDetail(comic, chapters, onChapter) {
    ...
    // BacaKomik genres: [{ title, slug }]
    const genres  = Array.isArray(comic.genres) ? comic.genres : [];

    const genreHTML = genres
      .map(g => {
        const name = typeof g === 'object' ? (g.title || g.name || '') : g;
        const slug = typeof g === 'object' ? (g.slug || '') : '';
        return name ? `<span class="badge badge-genre" style="cursor:pointer"
          onclick="browseController.genre='${slug}';browseController.load()">${name}</span>` : '';
      })
      .filter(Boolean)
      .join('');
  ...
  renderReader(containerId, images) {
    ...
    // BacaKomik chapter images: array of URL string
